...
janela1 = tkinter.LabelFrame(frame, text = 'Informações do Cliente',font=' ivy 10 bold',bg='#8FBC8F')
janela1.grid(row=1,column=0,padx=10,pady=5)


nome_label = ctk.CTkLabel(janela1,text='Nome do Cliente')
nome_label.grid(row=0,column=0,pady=0)
nome_entry = ctk.CTkEntry(janela1)
nome_entry.grid(row=1,column=0,)

cpf_label = ctk.CTkLabel(janela1,text='CPF: ')
cpf_label.grid(row=0,column=1)
cpf_entry = ctk.CTkEntry(janela1)
cpf_entry.grid(row=1,column=1)

tel_label = ctk.CTkLabel(janela1,text='Telefone')
tel_label.grid(row=0,column=2)
tel_entry = ctk.CTkEntry(janela1)
tel_entry.grid(row=1,column=2)

email_label=ctk.CTkLabel(janela1,text='Email')
email_label.grid(row=0,column=3)
email_entry=ctk.CTkEntry(janela1)
email_entry.grid(row=1,column=3)

cad_label=ctk.CTkLabel(janela1,text='Data do Cadastro')
cad_label.grid(row=0,column=4)
cad_entry=DateEntry(janela1)
cad_entry.grid(row=1,column=4)


cep_label=ctk.CTkLabel(janela1,text='Número do CEP:')
cep_label.grid(row=0,column=4)
cep_entry=ctk.CTkEntry(janela1)
cep_entry.grid(row=1,column=4)


rua_label = ctk.CTkLabel(janela1,text='Rua:')
rua_label.grid(row=3,column=0)
rua_entry = ctk.CTkEntry(janela1)
rua_entry.grid(row=4, column=0)


numero_label = ctk.CTkLabel(janela1,text='Número:')
numero_label.grid(row=3,column=1)
numero_entry = ctk.CTkEntry(janela1)
numero_entry.grid(row=4, column=1)

bairro_label = ctk.CTkLabel(janela1,text='Bairro:')
bairro_label.grid(row=3,column=2)
bairro_entry = ctk.CTkEntry(janela1)
bairro_entry.grid(row=4, column=2)

cidade_label = ctk.CTkLabel(janela1,text='Cidade:')
cidade_label.grid(row=3,column=3)
cidade_entry = ctk.CTkEntry(janela1)
cidade_entry.grid(row=4, column=3)

estado_label = ctk.CTkLabel(janela1,text='Estado:')
estado_label.grid(row=3,column=4)
estado_entry = ctk.CTkEntry(janela1)
estado_entry.grid(row=4, column=4)

ddd_label = ctk.CTkLabel(janela1,text='DDD:')
ddd_label.grid(row=3,column=5)
ddd_entry = ctk.CTkEntry(janela1)
...
xxx4=ctk.CTkButton(janela1,text='Consultar CEP:',command=cep, fg_color="green")
xxx4.grid(row=1,column=5)
...
janela2 = tkinter.LabelFrame(frame,text='Dados do Serviço',font=' ivy 10 bold', bg='#708090')
janela2.grid(row=2,column=0,padx=10,pady=5)

servico_label = ctk.CTkLabel(janela2,text='Servico Executado')
servico_label.grid(row=0,column=0)
servico_entry=ctk.CTkEntry(janela2)
servico_entry.grid(row=1,column=0)

data_servico_label = ctk.CTkLabel(janela2,text='Data do Servico')
data_servico_label.grid(row=0,column=1)
data_servico_entry = DateEntry(janela2)
data_servico_entry.grid(row=1,column=1)

valor_label = ctk.CTkLabel(janela2,text='Valor do Servico')
valor_label.grid(row=0,column=2)
valor_entry = ctk.CTkEntry(janela2)
valor_entry.grid(row=1,column=2)

parcela_label=ctk.CTkLabel(janela2,text='Quant. Parcelas')
parcela_label.grid(row=0,column=3)
parcelas_spinbox=ttk.Spinbox(janela2,from_=1,to=100,width=5,font=('Arial', 20),wrap=True)
parcelas_spinbox.grid(row=1,column=3)
...
valor_parcela_label=ctk.CTkButton(janela2,text='Valor das parcela', command=valor_parcela)
valor_parcela_label.grid(row=0,column=4)
valor_parcela_entry=ctk.CTkEntry(janela2)
valor_parcela_entry.grid(row=1,column=4)

entrada_label=ctk.CTkLabel(janela2,text='ENTRADA')
entrada_label.grid(row=0,column=5)
var_entrada=tkinter.StringVar(value='Sem entrada')
entrada_entry=customtkinter.CTkCheckBox(janela2,text='Com entrada',variable=var_entrada,onvalue='Com Entrada',offvalue='Sem Entrada')
entrada_entry.grid(row=1,column=5)
...
atendente_label.grid(row=2,column=1)
atendente_entry = ctk.CTkEntry(janela2)
atendente_entry.grid(row=3,column=1)

comissao_percentual_label = ctk.CTkLabel(janela2,text='% Comissão: ')
comissao_percentual_label.grid(row=2,column=2)
comissao_percentual_entry = ctk.CTkEntry(janela2)
comissao_percentual_entry.grid(row=3,column=2)

comissao_label=ctk.CTkLabel(janela2,text='Valor Comissão:')
comissao_label.grid(row=2,column=3)
comissao_entry=ctk.CTkEntry(janela2)
...
botao_verificar=ctk.CTkButton(janela2,command=verificar,text='Ver  Dados'.upper(),fg_color="blue")
botao_verificar.grid(row=4,column=0)

limpar_botao=ctk.CTkButton(janela2,command=limpar,text='Limpar Dados'.upper(),fg_color="green")
limpar_botao.grid(row=4,column=1,pady=10)

botao_verificar=ctk.CTkButton(janela2,command=atualizar_dados,text='ALTERAR DADOS'.upper(),fg_color="red")
botao_verificar.grid(row=4,column=2)


salvar_botao=ctk.CTkButton(janela2,command=cadastro,text='Salvar Dados'.upper(),fg_color="yellow")
salvar_botao.grid(row=4,column=4,pady=10)

salvar_botao=ctk.CTkButton(janela2,command=deletar_dados,text='Apagar Dados'.upper(),fg_color="black")
salvar_botao.grid(row=4,column=3,pady=10)

salvar_botao=ctk.CTkButton(janela2,text='xxxxxx'.upper())
salvar_botao.grid(row=4,column=5,pady=10,padx=8)
