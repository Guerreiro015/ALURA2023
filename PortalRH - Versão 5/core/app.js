...
function montarNotificacoes() {
  ...
  if (!baseInfo) notificacoes.push({ tipo:'warning', titulo:'Sem metadados da base', texto:'Quando importar a planilha, o THR registra data e nome do arquivo.' });
  notificacoes.push({ tipo:'info', titulo:'THR Enterprise v5.10', texto:'Home executiva, Alertas RH e Processos com indicadores estão ativos.' });
  return notificacoes;
}
...
function atualizarDossieTela() {
  ...
  const c = PortalRH.colaborador;
  if (!c) {
    box.innerHTML = '<div class="empty-history"><i class="fa-solid fa-address-card"></i><span>Selecione um colaborador no Dashboard para abrir o dossiê.</span></div>';
    return;
  }

  const historico = obterHistorico().filter(h => String(h.matricula) === String(c.matricula));
  const beneficios = historico.filter(h => ['Benefícios','Financeiro'].includes(MODULOS.find(m => m.id === h.moduloId)?.categoria || ''));
  const documentos = historico.filter(h => (MODULOS.find(m => m.id === h.moduloId)?.categoria || '') === 'Documentos');
  const porAno = {};
  historico.forEach(h => {
  ...
  const eventos = Object.entries(porAno).sort((a,b)=>b[0].localeCompare(a[0])).map(([ano, itens]) => `
    <div class="dossie-year"><h4>${UI.safe(ano)}</h4>${itens.slice(0,8).map(h => `
      <div class="dossie-event"><span></span><div><strong>${UI.safe(h.modulo)}</strong><small>${UI.safe(h.protocolo)} • ${UI.safe(h.data)} às ${UI.safe(h.hora)}</small></div></div>
    `).join('')}</div>
  `).join('');
  ...
  box.innerHTML = `
    <div class="dossie-hero">
      ...
      <div class="dossie-main">
        <span class="enterprise-kicker">Dossiê THR</span>
        <h2>${UI.safe(c.nome)}</h2>
        <p>${UI.safe(c.cargo || 'Cargo não informado')} • Matrícula ${UI.safe(c.matricula)}</p>
      </div>
    ...
    <div class="dossie-stats">
      ...
      <article><strong>${beneficios.length}</strong><span>Benefícios</span></article>
      <article><strong>${documentos.length}</strong><span>Documentos</span></article>
      <article><strong>${historico[0]?.data || '---'}</strong><span>Última ação</span></article>
    </div>

    <div class="dossie-layout">
      <section class="dossie-card">
        <h3><i class="fa-solid fa-id-card"></i> Dados Gerais</h3>
        <div class="dossie-data-grid">
          ${linha('Matrícula', c.matricula)}
          ${linha('CPF', c.cpf)}
          ${linha('Cargo', c.cargo)}
          ${linha('Empresa', c.empresa)}
          ${linha('Folha / Local', c.folha)}
      ...
      <section class="dossie-card">
        <h3><i class="fa-solid fa-layer-group"></i> Ações Rápidas</h3>
      ...
      <section class="dossie-card full">
        ...
        ${eventos || '<div class="empty-history"><i class="fa-solid fa-folder-open"></i><span>Nenhum evento registrado para este colaborador.</span></div>'}
      </section>

      <section class="dossie-card full">
        <h3><i class="fa-solid fa-paperclip"></i> Anexos e Documentos</h3>
        <div class="dossie-placeholder">
          <i class="fa-solid fa-file-shield"></i>
          <div><strong>Área preparada para anexos</strong><span>Na próxima etapa poderemos vincular RG, CPF, CNH, ASO, contratos e certificados ao colaborador.</span></div>
        </div>
      </section>
    </div>
  `;
...
async function importarBackupGeral(file) {
  ...
  try {
    ...
    if (!backup || !['Portal RH','THR Enterprise'].includes(backup.sistema)) {
...
function obterUsuarios() {
  ...
  // v5.10: remove o perfil Consulta, migra o antigo Suporte e inclui Regional.
  usuarios = usuarios
