...
def mostrar():
    ...
    # Crie a Treeview
    tree = ttk.Treeview(janela3, selectmode='extended', columns=tabela_head, show="headings", height=12)
   ...
   # Adicione barras de rolagem
    ...
    # Posicione a Treeview e as barras de rolagem
    tree.grid(column=0, row=0, sticky='nsew')
    vsb.grid(row=0, column=1, sticky='ns')
    hsb.grid(row=1, column=0, sticky='ew')

    # Configure o peso das linhas e colunas
    janela3.grid_rowconfigure(0, weight=1)
    janela3.grid_columnconfigure(0, weight=1)
...
def atualizar_dados():
   try:  
     ...
     confirma = askyesnocancel(title='Confirmação',message='Voce Realmemte quer alterar os dados ?')
     print(confirma)
     if confirma==True:
        ...
        messagebox.showinfo('CANCELADO', 'Alteração de dados cancelada pelo o Usuário')
        mostrar()
   except:
...
def deletar_dados():
  ...
  try:  
     ...
     confirma = askyesnocancel(title='Confirmação',message='Voce Realmemte quer DELETAR os dados ?')    
     if confirma==True:
